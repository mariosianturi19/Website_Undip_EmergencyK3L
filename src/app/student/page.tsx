...
export default function PanicButtonPage() {
  ...
  return (
    <StudentLayout>
      {/* Profile Card - Positioned at the top right with wider width */}
      <div className="absolute top-20 right-4 z-10 md:right-8 lg:right-12">
        <Card className="shadow-sm border-gray-200 min-w-80">
      ...
      <div className="container mx-auto px-4 max-w-md flex flex-col items-center pt-6">
        <div className="w-full text-center mb-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Emergency Assistance</h2>
          <p className="text-gray-600">
            Press the panic button to send an emergency alert with your location.
          </p>
        </div>
        ...
        <div className="w-full flex flex-col items-center">
          {isSending ? (
            ...
            <div className="relative panic-button-container">
              <button
                ...
                } text-white shadow-lg transition-all duration-200 panic-button`}
              >
              ...
              {/* Ripple effect for normal state */}
              {!isConfirming && !isSending && location && (
                <>
                  <span className="absolute inset-0 rounded-full bg-red-500 opacity-20 animate-ping-slow"></span>
                  <span className="absolute inset-0 rounded-full bg-red-500 opacity-10 animate-ping-slower" style={{ animationDelay: "0.5s" }}></span>
                </>
              )}
              
              {/* Pulse effect for confirmation state */}
              {isConfirming && !isSending && (
                <span className="absolute inset-0 rounded-full bg-yellow-500 opacity-30 animate-pulse"></span>
              )}
            </div>
          )}
        </div>
      ...
      {/* Custom CSS for animations - will be inlined in production */}
      <style jsx>{`
        .panic-button {
          position: relative;
          z-index: 10;
